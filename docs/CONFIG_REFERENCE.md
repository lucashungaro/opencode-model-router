...
> These settings (like anything in `tiers.json`) can also be placed in an overrides file — `~/.config/opencode/model-router-overrides.json` (global) or `<repo>/.opencode/model-router-overrides.json` (project) — and are deep-merged over the bundled defaults, so you don't have to edit the cached `tiers.json`. See the **Configuration** section of the README.
