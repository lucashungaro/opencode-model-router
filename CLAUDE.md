...
- **Most submodules under `src/guard/`, `src/verify/`, `src/escalate/`, `src/telemetry/` (and `src/commands/output.ts`) are PURE** — no `fs`, network, SDK, or `process.env`; side effects are injected via `deps` objects. The deliberate exception is **`src/verify/wiring.ts`**, the impure Layer-2 corner (exec/fs/opencode client) built by `createVerificationWiring(...)` and consumed by `index.ts`. Coverage thresholds enforce the design (guard/verify/router ≥90% branch, escalate/telemetry ≥95%). Keep the pure modules pure — push new I/O into `index.ts` or `wiring.ts`.
...
- `src/verify/` — Layer 2 acceptance gate. `gate.ts` is the single accept/verify decision point (fail-closed; producer ≠ grader; grader ≥ producer tier). `dod.ts` (Definition-of-Done schema), `deterministic.ts`, `checker.ts` (graded), `dispatch.ts`, `types.ts`, and `wiring.ts` (the impure seams: exec/fs/grader-dispatch/gate-deps/scorecard via `createVerificationWiring`).
- `src/commands/output.ts` — pure renderers for every slash command (config/data in → markdown out). The `command.execute.before` handlers in `index.ts` decide + persist, then call these.
- `src/escalate/` — Layer 3 quality-escalation ladder (retry → fast→medium→heavy, bounded by attempt/cost ceilings).
- `src/telemetry/trajectory.ts` — per-subagent scorecard (record-only; recording is skipped unless `MODEL_ROUTER_TRAJECTORY_DEBUG=1`).
