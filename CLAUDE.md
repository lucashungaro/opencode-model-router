...
- **Two user-config layers, merged in `loadConfig()`:** (1) `~/.config/opencode/model-router-overrides.json` (`overridePath()`) is deep-merged over the bundled `tiers.json` — this is how users customize models/tiers without editing the cached package file; (2) the state file then overlays the active preset/mode/enforcement. Override read/parse/validation failures `console.warn` and fall back to the bundled config (never throw) so a bad override can't brick startup. `deepMerge` replaces arrays/scalars and recurses into plain objects.
